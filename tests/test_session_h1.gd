...
func _registry() -> RefCounted:
	...
	var result: Dictionary = registry.initialize_fixture_catalog(approved_layout_ids)
...
func _test_content_registry() -> void:
	...
	var selected: Dictionary = registry.resolve_layout("fixture.variable_30x40_single")
	_assert(bool(selected.get("valid", false)), "explicit variable layout selection resolves")
	_assert(selected.get("layout_ref", {}).get("layout_id", "") == "fixture.variable_30x40_single", "selection returns the selected stable identity")
	_assert(registry.get_layout_ids().size() == 3, "registry exposes only its three approved identities")
	...
	var wrong_version: Dictionary = registry.resolve_layout("fixture.variable_30x40_single", 99)
	...
	var wrong_fingerprint: Dictionary = registry.resolve_layout("fixture.variable_30x40_single", -1, "0".repeat(64))
	...
	var aliases: Array[String] = ["A"]
	var alias_result: Dictionary = alias_registry.initialize_fixture_catalog(aliases)
...
func _test_readiness_barrier() -> void:
	...
	var begin: Dictionary = coordinator.begin("fixture.mixed_3x3", registry)
	...
	_assert(coordinator.can_accept_input() and ready_events == ["fixture.mixed_3x3"], "session-ready publishes once after the barrier")
	coordinator.dispose()
