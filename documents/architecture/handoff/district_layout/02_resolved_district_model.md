...
**Current amendment, 2026-09-10:** [ADR 36](../../decisions/36_district_zone_spatial_snapshot_and_zone_mutation.md), as corrected before implementation acceptance by [ADR 37](../../decisions/37_district_construction_annex_schema.md), amends only this frozen state schema to local version 3. The root remains Save V2; H1/H2 definition fingerprints and resolved-layout goldens do not change. Implementation and new evidence are not asserted, and prior evidence proves neither amendment.
...
- `DistrictState` has exactly `state_schema_version`, `district_revision`, `layout_id`, `layout_definition_version`, `definition_fingerprint`, `plot_states`, `street_segment_states`, `arrival_source_states`, `demolished_fixed_occupant_ids`, `construction_schema_version`, `construction_revision`, `construction_records`, and `manual_door_edges`. Unknown or missing keys reject; `manual_door_records` is forbidden. Types and prior-field invariants remain as stated here, `state_schema_version=3`, and `construction_schema_version=1`.
...
- `FloorState` has exactly `floor_id`, signed integer `elevation`, and normalized ordered cell sets `acquired_cells`, `constructed_cells`, and `explicit_circulation_cells`. This is exactly the prior shape plus `explicit_circulation_cells`; it exists only when at least one set is nonempty. Every constructed cell must be in effective acquired rights and satisfy definition rights and physical/progression caps; construction without effective acquired rights rejects. Every explicit-circulation cell must be constructed and acquired. Generic constructed or unzoned space is never inferred as circulation. Elevation `0` may contain `acquired_cells` as tile-level floor-space state, independently of section land rights, including partial sets. Section ownership remains separately derived and is neither granted nor revoked by elevation-0 acquired cells.
...
Each `manual_door_edges` record has exactly `endpoint_a` and `endpoint_b`, each an exact ADR 31 `FloorCellAddress` `{runtime_plot_id:string, signed_elevation:int, local_cell:{x:int,y:int}}`. Endpoints have the same `runtime_plot_id` and `signed_elevation`, are distinct and orthogonally adjacent, and are canonical ascending by `(runtime_plot_id,signed_elevation,local_cell.y,local_cell.x)`. The collection is sorted by endpoint pair and duplicate-free. Presence means a committed door; zone identity and typology are not duplicated.

ADR 37 is normative for the exact construction annex and `ConstructionCellAddress` schema. `construction_records` is duplicate-free and sorted by `construction_id`, permits only stairs, elevators, and Operations Rooms, and obeys the exact geometry, canonical-address, occupied-union, no-overlap, ownership/cap/policy, and restore invariants there. Corridors never have records: active corridors exist only in `explicit_circulation_cells`, with deterministic unsaved identity from exact floor-cell and layout identity. `constructed_cells` includes active explicit circulation and every retained record's occupied union; generic unzoned construction is not inferred as corridor.

Current fixed occupancy is the immutable snapshot occupant set minus `DistrictState.demolished_fixed_occupant_ids`, covering decorative non-Plot occupants and Plot occupants uniformly. Demolition applies atomically to one whole stable runtime fixed-occupant ID across all of its elevation masks; partial elevation demolition is unrepresentable. `district_revision` is the sole district concurrency revision: no child or annex record has a concurrency revision. `construction_revision` is nonnegative, cannot exceed `district_revision`, and increments exactly once in a successful District commit changing `construction_records` or any `explicit_circulation_cells`; otherwise it is unchanged. Consumers bind it to enclosing District layout/revision/commit identity and never commit against it alone. State collections use ascending stable-ID order; within each Plot, `section_state_overrides` use runtime section ID, `floor_states` use signed elevation ascending then floor ID, and acquired, constructed, and explicit-circulation cell sets use row-major `(y,x)`. Duplicate IDs, elevations, edges, records, or cells reject, and empty override records are forbidden. This is the complete district persistence inclusion. Derived Active/Fully Owned flags, rights masks, intersections, graphs, geometry, transforms, reservations, and caches are not mutable district state.

Local DistrictState v2, older, and unknown schemas reject before staging. There is no converter or compatibility adapter; this pre-release strict rejection does not rename or version the root Save V2 envelope.
