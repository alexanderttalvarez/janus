...
- **2026-08-31 Fixture C Initial Ownership Decision; rationale corrected 2026-09-08 under ADR 33:** Initial ownership remains exactly `{market_entry,station_entry,garden_entry}`. These are the only initially owned sections, a proof-only exception to the single-owned-Plot production start. They do not form a geometrically connected Plot union; `district_layout/H7` handles active-slot components. Whole plot IDs `market_hall` and `garden_plot` are not ownership values. No semantic record, mask or fingerprint is changed by this rationale correction.
